...
#
base_toda <- purrr::map(1:as.numeric(
  page |>
...
\(numero) {
  prefixo <- page |>
    rvest::html_elements('div select') |> purrr::pluck(numero)
...
form_data <- gsub("\\\\u00",
  ...
  tabela <- utils::read.csv2(paste0('http://tabnet.datasus.gov.br/cgi/',
                                          response |>
